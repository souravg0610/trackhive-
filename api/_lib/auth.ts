...
export function requireUser(req: VercelRequest, res: VercelResponse): JwtPayload | null {
  const u = getUser(req);
  if (!u) { res.status(401).json({ success: false, message: 'Unauthorized' }); return null; }
  return u;
}
...
export function fail(res: VercelResponse, msg: string, status = 500) {
