...
export default async function(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    let q = sb.from('leaves').select('*', { count: 'exact' }).eq('company_id', user.companyId).order('created_at', { ascending: false });
  ...
  if (req.method === 'POST') {
    ...
    if ('leaves' === 'leaves') {
      ...
      const { data, error: e } = await sb.from('leaves').insert({ ...body, id: id2, company_id: user.companyId, total_days: totalDays, status: 'pending', applied_on: new Date().toISOString().split('T')[0] }).select().single();
      if (e) return fail(res, e.message); return ok(res, data, 201);
    }
    const { data, error: e } = await sb.from('leaves').insert({ ...body, company_id: user.companyId }).select().single();
    if (e) return fail(res, e.message); return ok(res, data, 201);
  }
  if (id && (req.method==='PUT'||req.method==='PATCH')) {
    const body = req.body; let update = { ...body };
    if ('leaves'==='leaves' && body.action) { if(body.action==='approve')update={status:'approved',approved_by:user.name,approved_at:new Date().toISOString()};if(body.action==='reject')update={status:'rejected',rejection_reason:body.reason}; }
  ...
  if (id && req.method==='DELETE') { await sb.from('leaves').delete().eq('company_id',user.companyId).eq('id',id); return ok(res,{deleted:true}); }
