...
export default async function(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const { data, error: e } = await sb.from('attendance').select('*').eq('company_id', user.companyId).eq('date', today).order('created_at', { ascending: false });
  ...
  if (req.method === 'POST') {
    ...
    if (!empId) {
      const { data: emp } = await sb.from('employees').select('id,name,department').eq('company_id', user.companyId).ilike('email', user.email).maybeSingle();
      if (emp) { empId = emp.id; empName = emp.name; dept = (emp as any).department||dept; } else empId = user.sub;
    }
    const id = `ATT-${user.companyId}-${empId}-${today}`;
    if (action === 'punch-in') {
      const { data: existing } = await sb.from('attendance').select('check_in_time').eq('id', id).maybeSingle();
      if (existing?.check_in_time) return fail(res, 'Already punched in today', 409);
    ...
    if (action === 'punch-out') {
      ...
      if (rec?.check_in_time) {
        const pt = (t: string) => { const [hm, ap] = t.split(' '); let [h,m] = hm.split(':').map(Number); if(ap==='PM'&&h!==12)h+=12; if(ap==='AM'&&h===12)h=0; return h*60+m; };
        const diff = pt(time)-pt(rec.check_in_time as string);
        workingHours = `${Math.floor(diff/60)}h ${diff%60}m`;
      }
      const { data, error: e } = await sb.from('attendance').update({ check_out_time: time, working_hours: workingHours, location: location||'' }).eq('id', id).select().single();
      if (e) return fail(res, e.message);
      return ok(res, data);
    }
